...
public class AI {
	...
	/**
	 * Callback set by ChessBoard to show in-game overlays instead of Swing dialogs.
	 */
	...
	public static String alphaBeta(int depth, int beta, int alpha, String move, int player) {
		...
		for (int i = 0; i < list.length(); i += 5) {
			...
			//////////////////////////////////
			// if(depth==4) System.out.println("move: "+move+" ,value: "+value+", alpha:
	...
	public static void makeMove(String move) {
		...
		// System.out.println(Character.getNumericValue(move.charAt(0)) + "\t" +
	...
	private static boolean Shoulder(int oldRow, int oldColumn, int curRow, int curColumn, String faction,
		...
		if ((curRow + curColumn) % 2 == 0) {
			...
			try {
				if ((chessBoard[curRow + 1][curColumn + 1] == enemy)
						&& (chessBoard[curRow - 1][curColumn - 1] == enemy)) {
			...
			if (currentOpeningPosition.possible) {
				...
				if (!currentOpeningPosition.relatedPoint.isEmpty()) {
					...
					for (Point p : currentOpeningPosition.relatedPoint) {
						...
						// AIBoard.addActor(new OpeningSign(new
						// Vector2(4-currentOpeningPosition.getRow(),
						// 4-currentOpeningPosition.getColumn())));
					}
				}
			}
		}
	...
	public static boolean isWinLose(String faction) {
		...
		if (humanLost) {
			if (onGameEnd != null)
				onGameEnd.onComputerWins();
		}
		return humanLost;
	}

	/**
	 * Called in normal board state (no flip) — "A" = human pieces, "a" = computer
	 * pieces.
	 * If no "a" found → computer pieces are all gone → HUMAN wins.
	 */
	 ...
	 * If no "A" found → human pieces are all gone → COMPUTER wins.
	 */
	public static boolean isHumanLoseNormal() {
		boolean humanLost = true;
		for (short i = 0; i < 5; i++)
			for (short j = 0; j < 5; j++) {
				if ("A".equals(chessBoard[i][j]))
